...
#' loss_deriv_expectile <- function(x, c, delta) { -2 * ifelse(x > c, delta, 1 - delta) * (x - c) }
#'
...
fit_nonpar_square <- function(X, Y, X0, d_tau, tau, h_x_loc, h_x_cdf, h_y_cdf) {
  ...
  for (i in seq_len(m)) {
    ...
    F_hat <- ConditionalCDF(Y_eval = Y, X_eval = x0_mat,
                            Y_train = Y, X_train = X,
    ...
    fit <- stats::lm.wfit(x = X_loc[valid, , drop = FALSE],
