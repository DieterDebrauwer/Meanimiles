...
#' d_tau_minvar <- function(u, tau) { (tau + 1) * u^tau }
#' lossDerivativeExpectiles <- function(x, c, delta) { -2 * ifelse(x > c, delta, 1 - delta) * (x - c) }
#'
#' # 1. Square Loss
#' MeanimileLinearRegression(X, Y, d_tau = d_tau_minvar, tau = 2,
...
fit_linear_square <- function(X, Y, d_tau, tau, h_x, h_y) {
  ...
  valid_S2 <- !is.na(W_S2)
  fit_S2 <- stats::lm.wfit(x = X_G[idx_S2, , drop = FALSE][valid_S2, , drop = FALSE],
  ...
  valid_S1 <- !is.na(W_S1)
  fit_S1 <- stats::lm.wfit(x = X_G[idx_S1, , drop = FALSE][valid_S1, , drop = FALSE],
...
fit_linear_general <- function(X, Y, d_tau, tau, h_x, h_y, lossDerivative, delta) {
  ...
  solve_fold <- function(idx_eval, idx_train) {
    F_hat <- ConditionalCDF(Y[idx_eval], X[idx_eval, , drop = FALSE],
